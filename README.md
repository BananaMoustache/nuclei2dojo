...
* `proc/nuclei_runner.py` – wrappers around `nuclei` for **single** and **list** mode, including severity, tags, rate limit, concurrency, and template exclusion. 
...
* If `--profile httpx` is used:
  ...
  * Technologies are mapped to Nuclei **tags** (e.g. PHP, WordPress, IIS, jQuery, Cloudflare → related tags).
